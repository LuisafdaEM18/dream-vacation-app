...
app.post('/api/destinations', async (req, res) => {
  ...
  try {
    ...
    // Extrayendo los values del objeto languages (ej. { "spa": "Spanish" } -> "Spanish")
    // Note: el formato puede diferir en v4, veamos: v4 arroja un array: [{"iso639_1":"es","iso639_2":"spa","name":"Spanish","nativeName":"Español"}]
    // o un objeto? En la consulta anterior v4 de colombia arrojó: "languages":[{"iso639_1":"es","iso639_2":"spa","name":"Spanish","nativeName":"Español"}]
    let languages = null;
    if (Array.isArray(countryInfo.languages)) {
        languages = countryInfo.languages.map(l => l.name).join(', ');
    } else if (countryInfo.languages) {
    ...
    const [result] = await pool.query(
      ...
      [country, countryInfo.capital[0], countryInfo.population, countryInfo.region, languages]
    );
    res.status(201).json({ id: result.insertId, country, capital: countryInfo.capital[0], population: countryInfo.population, region: countryInfo.region, languages });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Internal server error' });
  }
